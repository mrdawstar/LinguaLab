  ...
  return (
    <DashboardLayout requiredRole="manager">
      <div className="glass-card p-4 sm:p-6">
        ...
        >
          <div className="flex items-center justify-between rounded-2xl border border-border/60 bg-background/80 p-3 shadow-sm backdrop-blur">
            <div>
          ...
          <div className="space-y-3">
            {getLessonsForDay(selectedDay || new Date()).map((lesson, index) => {
              ...
              return (
                ...
                >
                  <div className={cn(
                    ...
                    )}>{lesson.start_time.slice(0, 5)}</span>
        ...
        <div className="hidden lg:block overflow-x-auto -mx-4 sm:mx-0">
          <div className="min-w-[800px] rounded-2xl border border-border/50 bg-background/50 p-2 sm:px-0">
            {/* Header */}
                  ...
                  {weekDays.map((day, dayIndex) => {
                    ...
                    return (
                      <div
                        ...
                        className={cn(
                          'relative min-h-[50px] sm:min-h-[60px] rounded-lg border border-dashed border-border/50 p-1 cursor-pointer transition-colors hover:bg-muted/50',
                        ...
                        {slotLessons.map((lesson) => {
                          ...
                          return (
                            <div
                              ...
                              style={{ backgroundColor: isCompleted ? '#10b981' : (teacher?.calendar_color || '#10b981') }}
                              onClick={(e) => {
                                e.stopPropagation();
                                setSelectedLesson({
                                  lesson,
                ...
                <div className="flex items-start justify-between gap-3">
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">
                      {format(new Date(selectedLesson.lesson.date), 'EEEE, d MMMM yyyy', { locale: pl })}
                    </p>
                  </div>
                  <span
                    className={cn(
                      "rounded-full px-2.5 py-1 text-xs font-semibold",
                      selectedLesson.lesson.is_completed
                  ...
                  >
                    {selectedLesson.lesson.is_completed ? 'Zakończone' : 'Zaplanowane'}
                  </span>
                </div>
                <div className="mt-3 flex flex-wrap items-center gap-2 text-sm">
                  <span className="rounded-full border border-border/60 bg-background px-3 py-1 text-xs font-semibold">
                    {selectedLesson.lesson.start_time.slice(0, 5)} - {selectedLesson.lesson.end_time.slice(0, 5)}
                  </span>
                  <span className="rounded-full border border-border/60 bg-background px-3 py-1 text-xs font-semibold">
                ...
                {selectedLesson.teacher && (
                  <div className="flex items-center gap-3 rounded-xl border border-border/60 bg-background/80 p-3">
                    <span
                      ...
                      style={{ backgroundColor: selectedLesson.teacher.calendar_color || '#10b981' }}
                    />
